// __tests__/frontend/SubmitOvertime.test.tsx
import React from 'react';
import { render } from '@testing-library/react';
import { fireEvent } from '@testing-library/dom'; // Import fireEvent from @testing-library/dom
import userEvent from '@testing-library/user-event';
import SubmitOvertime from '@/app/submit/overtime/page'; // Adjust the import path if necessary
...
describe('SubmitOvertime Form', () => {
  ...
  it('renders the form with required fields', () => {
    const { getByLabelText, getByTestId, getByRole } = render(<SubmitOvertime />);

    // Check for required form elements (adjust based on actual form structure)
    expect(getByLabelText(/Date/i)).toBeInTheDocument();
    expect(getByLabelText(/Start Time/i)).toBeInTheDocument();
    expect(getByLabelText(/End Time/i)).toBeInTheDocument();
    ...
    expect(getByRole('button', { name: /Submit Overtime/i })).toBeInTheDocument();
  });

  it('displays validation errors for missing required fields on submit', async () => {
    const { getByRole } = render(<SubmitOvertime />);

    // Attempt to submit the form without filling required fields
    await userEvent.click(getByRole('button', { name: /Submit Overtime/i }));
    ...
    // expect(screen.getByText(/Start Time is required/i)).toBeInTheDocument(); // Use getByText from screen if needed
  ...
  it('handles successful form submission', async () => {
    ...
    global.fetch = jest.fn(() =>
      Promise.resolve({
        ok: true,
        json: () => Promise.resolve({ message: 'Overtime submitted successfully' }),
      }) as Promise<Response>
    );

    const { getByLabelText, getByRole, getByTestId } = render(<SubmitOvertime />);
    ...
    await userEvent.type(getByLabelText(/Start Time/i), '18:00');
    await userEvent.type(getByLabelText(/End Time/i), '20:00');
    await userEvent.type(getByLabelText(/Reason/i), 'Completed project tasks');
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      '/api/overtime', // Adjust endpoint if necessary
    ...
    expect(toast.success).toHaveBeenCalledWith('Overtime submitted successfully'); // Adjust message if necessary
  ...
  it('handles input field interactions', async () => {
    const { getByLabelText } = render(<SubmitOvertime />);

    // Test typing in text inputs
    await userEvent.type(getByLabelText(/Reason/i), 'Urgent task completion');
    expect(getByLabelText(/Reason/i)).toHaveValue('Urgent task completion');

    // TODO: Add tests for DatePicker and TimePicker components if they are not standard HTML inputs
  });

  it('displays specific validation error messages', async () => {
    const { getByRole, findByText } = render(<SubmitOvertime />);
    ...
    expect(await findByText(/Start Time is required/i)).toBeInTheDocument();
    expect(await findByText(/End Time is required/i)).toBeInTheDocument();
    expect(await findByText(/Reason is required/i)).toBeInTheDocument();
    // Add checks for other specific validation rules (e.g., end time before start time)
  });

  it('handles file upload and removal', async () => {
    const { getByTestId } = render(<SubmitOvertime />);
